...
from tests.identity.test_service import EMAIL, IdentityHarness
from tests.identity.test_service import identity as identity


async def issue_pair(
    identity: IdentityHarness, email: str = EMAIL
) -> TokenPair:
    await identity.reset_cooldown(email)
    await identity.service.sign_in_with_email(email)
    return await identity.service.verify_sign_in_code(
        email, identity.email.last_code
    )


async def recovery_code(identity: IdentityHarness) -> str:
...
async def test_recovery_revokes_all_old_tokens_but_preserves_account(
    ...
    old_claims = identity.verifier.verify(
        first.access_token, TokenPurpose.ACCESS
    )
    second_claims = identity.verifier.verify(
        second.access_token, TokenPurpose.ACCESS
    )
    ...
    new_claims = identity.verifier.verify(
        recovered.access_token, TokenPurpose.ACCESS
    )
    ...
    for pair in (first, second):
        claims = identity.verifier.verify(
            pair.access_token, TokenPurpose.ACCESS
        )
    ...
    renewed = await identity.service.refresh_token_pair(
        recovered.refresh_token
    )
    renewed_claims = identity.verifier.verify(
        renewed.access_token, TokenPurpose.ACCESS
    )
...
async def test_recovery_requires_live_email_code(
    identity: IdentityHarness, invalid: str
) -> None:
...
async def test_unconfirmed_recovery_can_be_retried_with_new_code(
    identity: IdentityHarness, monkeypatch: pytest.MonkeyPatch, committed: bool
) -> None:
    pair = await issue_pair(identity)
    code = await recovery_code(identity)
