...
@pytest.mark.parametrize(
    "missing",
    ["exp", "iat", "sub", "role", "type", "jti", "iss", "aud", "gen"],
)
...
def test_stateless_token_lifetimes_and_purposes() -> None:
    ...
    claims = UserClaims(
        id=uuid4(), role=UserRole.PERMANENT, token_generation=uuid4()
    )
    ...
    for token, purpose, lifetime in (
        ...
        assert (
            verifier.verify(token, purpose).token_generation
...
def test_registered_token_rejects_invalid_generation(
    generation: object,
) -> None:
    generator = TokenGenerator(
        TokenGeneratorSettings(secret_key=SecretStr(SECRET))
